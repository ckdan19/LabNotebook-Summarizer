...
Runs all seven lab-notebook subagents in parallel, collects their summaries, and compiles them into a single Markdown file organized by source. The window defaults to the last 7 days, but the user can request any window ("a full lab digest for the last 14 days").
...
2. **Launch all seven subagents in parallel** in a single message (do NOT run them one at a time). Substitute `days` into the prompt:
...
3. **Load digest state and exclude already-covered posts.** The digest tracks which posts have appeared in a previous digest so no post is ever covered twice.
   ...
   - **Read the state file** at `digests/.digest-state.json` (path is relative to the repository root). It has this shape:
     ...
     `digested_urls` is the set of every post URL that has appeared in any previous digest, across all seven sources.
...
> [N] of 7 sources had activity in the last [days] days. [M] had none.
...
[paste the kathleen-notebook-agent summary verbatim here]
...
7. **Write the Cross-Notebook Patterns & Connections section** by analyzing only the seven compiled summaries (not the original source content). Look for three specific types of connections:
...
9. **Write the Data & Figures section** by consolidating the figure and data/repository links that the seven subagents already surfaced in their per-source summaries. This is a reorganization step only — it uses the compiled summaries you already have and issues no new fetches, subagent runs, or external calls.
