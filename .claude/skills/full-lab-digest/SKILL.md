...
3. **Tally activity**: count how many sources returned at least one post ("had activity") and how many reported no posts ("no activity").

4. **Determine the date range**: use the `days`-day window ending on today's date.
...
5. **Assemble the combined digest** using this exact structure:
...
[cross-notebook analysis — see step 6]

---

## Literature Connections

[literature search results — see step 7]
...
```
   ...
   - If a subagent reported no activity, write a single line under that section's header: `_No posts in the last [days] days._`

6. **Write the Cross-Notebook Patterns & Connections section** by analyzing only the five compiled summaries (not the original source content). Look for three specific types of connections:
...
7. **Write the Literature Connections section** by running the `literature-connector` skill for each notable finding. This step involves multiple external API calls and will take longer than the rest of the digest — that is expected.
...
9. **Return the file path** to the user in the main conversation.
